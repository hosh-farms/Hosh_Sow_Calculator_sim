...
# streamlit run sowcalc_streamlit_final_fixed.py
...
# (kept your logic; fixed cumulative cash flow start)
# -------------------------------
def sow_rotation_simulator(
    ...
    # --- finalize monthly DataFrame
    df_month = pd.DataFrame(monthly_data)

    # Yearly summary (kept your grouping)
    ...
    # -------------------------
    # CUMULATIVE columns and ROI/CAGR (FIXED cumulative cash flow)
    # -------------------------
    ...
    # cash series (use unrounded for accurate sums)
    cash_series = df_month['Monthly_Cash_Flow'].astype(float).fillna(0.0)

    # Build cumulative cash flow starting at -(initial_investment)
    running = -initial_investment
    cumulative_cash = []
    for cf in cash_series:
        running += cf
        cumulative_cash.append(running)

    # cumulative profit (monthly_profit cumulative)
    cumulative_profit = df_month['Monthly_Profit'].astype(float).cumsum()

    # add columns to df_month (rounded to integers for display)
    df_month['Cumulative_Cash_Flow'] = [round(x, 0) for x in cumulative_cash]
    df_month['Cumulative_Profit'] = cumulative_profit.round(0)

    final_cumulative_cash_with_capital = float(cumulative_cash[-1]) if len(cumulative_cash) > 0 else -initial_investment
    final_cumulative_profit = float(cumulative_profit.iloc[-1]) if len(cumulative_profit) > 0 else 0.0
    ...
    # Realized CAGR on cash-only flows (money multiple based on cash returned relative to initial investment)
    years = months / 12.0
    ...
    be_idx = [i for i, v in enumerate(cumulative_cash) if v >= 0]
    ...
    if be_month is not None:
        ...
        profit_after_break_even = rem.sum() if not rem.empty else 0.0
        avg_profit_after_breakeven = rem.mean() if not rem.empty else 0.0

    # Return everything (kept same order/names as you used)
    return (
        df_month,
        df_year,
        total_sow_cost,
...
st.write("Monthly Summary (first rows)")
...
df_plot1 = df_month.loc[df_month['Month'] <= 24, ["Month"] + [c for c in cost_components if c in df_month.columns] + ["Revenue"]].copy()

df_costs_melt = df_plot1[["Month"] + [c for c in cost_components if c in df_month.columns]].melt(id_vars="Month", var_name="Cost Component", value_name="Value")
...
existing_cost_components = [c for c in cost_components if c in df_month.columns]
total_costs = df_month[existing_cost_components].sum().reset_index()
...
st.write("Done — cumulative cash flow now **starts at** -(initial capital + working capital) and accumulates monthly cash flows step-by-step.")
