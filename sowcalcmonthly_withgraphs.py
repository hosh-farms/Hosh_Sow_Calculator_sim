# save this as sowcalc_streamlit_final.py and run:
...
# (kept your logic; I only added cumulative columns, ROI/CAGR and plotting)
# -------------------------------
def sow_rotation_simulator(
    ...
    sow_life_years=4,
    loan_amount=4000000,
    interest_rate=0.121,           # pass as decimal inside function (UI will pass % / 100)
    loan_tenure_years=5,
    moratorium_months=0,
    medicine_cost=10000,
    ...
    cumulative_cash_with_capital = 0   # we'll set proper start later
    total_pigs_born = 0
    total_pigs_sold = 0

    first_sale_cash_needed = 0         # working capital required until first sale
    first_sale_done = False

    # We will store monthly_cash_flow (revenue - operating - loan_payment)
    # and monthly_profit (revenue - operating) for cumulative profit calculations.
    ...
    for month in range(1, months + 1):
        ...
        # Track first sale working capital (running total until first sale)
        ...
        # Monthly profit (profit before loan & depreciation)
        monthly_profit = revenue - total_operating_cost
        # Monthly cash flow (cash movement considering loan EMI)
    ...
    # YEARLY summary (keeps your previous grouping)
    ...
    # Recompute loan amort (for total_interest_paid) from original loan terms
    loan_balance = loan_amount
    total_interest_paid = 0.0
    ...
    # -------------------------
    # initial_investment (capital + working capital until first sale)
    initial_capital = total_capital                       # shed + sows
    initial_working_capital = first_sale_cash_needed      # working capital required up front (running sum till first sale)
    ...
    # 1) cumulative_working_cap_flow starts at -first_sale_cash_needed and then accumulates monthly cash flows
    # 2) cumulative_cash_with_capital starts at -(total_capital + first_sale_cash_needed) and then accumulates monthly cash flows
    ...
    cumulative_profit_excl_capital = df_month['Monthly_Profit'].astype(float).cumsum()   # monthly profit cumulative (no capital)

    # Add new columns to df_month (rounded)
    ...
    growers_remaining_value = animals_left * final_weight * sale_price   # market value estimate for growers remaining

    final_assets_value = shed_remaining_value + sows_remaining_value + growers_remaining_value

    # ROI (cash only, excludes assets) — expressed as %
    # ROI_cash = (final cash returned to owner) / initial_investment * 100
    # final cash returned = sum(monthly_cash_flow)  (since initial outflows are separate)
    ...
    # ROI including assets (liquidate remaining assets at final_assets_value)
    roi_with_assets_pct = ((total_cash_returned + final_assets_value) / initial_investment) * 100 if initial_investment > 0 else float('nan')

    # Realized CAGR on realized cash flows (exclude assets) — annualized return on cash-only outcome
    # We use: money_multiple = total_cash_returned / initial_investment
    # realized_cagr = money_multiple ** (1/years) - 1, but this only makes sense if both >0
    years = months / 12.0
    ...
    if initial_investment > 0 and total_cash_returned > 0 and years > 0:
        ...
        # if money_multiple <= 0, CAGR not meaningful
        if money_multiple > 0:
            realized_cagr = (money_multiple ** (1.0/years) - 1.0) * 100.0
        else:
            realized_cagr = float('nan')

    # Total ROI (older definition you had: using cumulative cash with capital)
    ...
    breakeven_idx = df_month[df_month['Cumulative_Cash_Flow'] >= 0].index
    if len(breakeven_idx) > 0:
        be_month = int(df_month.loc[breakeven_idx[0], 'Month'])
    else:
        be_month = None
    ...
    if be_month:
        remaining_profits = df_month.loc[df_month['Month'] >= be_month, 'Monthly_Profit']
        avg_profit_after_breakeven = remaining_profits.mean() if not remaining_profits.empty else 0.0

    # Pack results to return in the same order your UI expects
    # (df_month, df_year, total_sow_cost, shed_cost, first_sale_cash_needed, total_pigs_sold,
    #  total_pigs_born, animals_left, cumulative_cash_flow, total_interest_paid,
    #  break_even_month, profit_after_break_even, average_monthly_profit, avg_profit_after_breakeven,
    #  total_crossings, roi_with_assets, realized_cagr, total_roi_pct)
    return (
        df_month,
        df_year,
        total_sow_cost,
        ...
        total_pigs_born,
        animals_left,
        final_cumulative_cash_with_capital,    # cumulative_cash_flow scalar (includes capital & WC start)
        total_interest_paid,
        be_month,
...
with col1:
    st.write("Monthly Summary (first 60 rows shown)")
    st.dataframe(df_month.head(120))  # show months with our new columns
with col2:
...
if break_even_month:
    st.write(f"Break-even Month (cumulative incl. capital): {break_even_month}")
else:
...
# ROI & CAGR outputs
st.write("---")
st.write(f"ROI (Cash only, excludes assets): {roi_cash_pct:.2f}%")
st.write(f"ROI (Including asset liquidation): {roi_with_assets_pct:.2f}%")
if math.isnan(realized_cagr):
    st.write("Realized CAGR (cash only): Not meaningful / NaN for these numbers")
else:
    st.write(f"Realized CAGR (cash only): {realized_cagr:.2f}%")
st.write("---")
...
# Plot 1: Revenue vs Total Costs (stacked) - show first 24 months only as you requested
...
# Break-even rule (based on Cumulative Cash Flow includes capital)
rule = None
if break_even_month:
...
# Extra: ROI & CAGR over time chart (cumulative ROI and realized CAGR)
st.write("6) Cumulative ROI (cash only) & Realized CAGR (over time)")
total_capital_for_chart = initial_capital
months_arr = df_month['Month']
cumulative_cash = df_month['Cumulative_Cash_Flow']  # includes capital
cumulative_roi_pct_over_time = ((df_month['Cumulative_Cash_Flow'] / initial_investment) * 100).fillna(0)
# realized CAGR over time using running years (guard against divide by zero)
cumulative_cash_only = ((-initial_investment) + df_month['Monthly_Cash_Flow'].cumsum())
with pd.option_context('mode.use_inf_as_na', True):
    yrs_elapsed = months_arr / 12.0
    realized_cagr_over_time = pd.Series([None]*len(months_arr))
    for i, m in enumerate(months_arr):
        yrs = (m / 12.0)
        if yrs > 0:
            final_cash = df_month.loc[:i, 'Monthly_Cash_Flow'].sum()
            if initial_investment > 0 and final_cash > 0:
                mult = final_cash / initial_investment
                if mult > 0:
                    realized_cagr_over_time.iloc[i] = (mult ** (1.0/yrs) - 1.0) * 100.0
                else:
                    realized_cagr_over_time.iloc[i] = None
            else:
                realized_cagr_over_time.iloc[i] = None
...
# Done
st.write("App finished. If anything seems off with the figures (NaN or unexpected), adjust inputs or ask me to re-check calculation choices.")
